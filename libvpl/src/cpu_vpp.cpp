...
vplStatus CpuWorkstream::InitVPP(vplm_mem *vm) {
  ...
  m_vppFrameInfoIn.NumPlanes = (uint32_t)CpuWorkstream::GetImagePlanesNum(m_vppFrameInfoIn.FourCC);
  ...
  m_vppFrameInfoOut.NumPlanes = (uint32_t)CpuWorkstream::GetImagePlanesNum(m_vppFrameInfoOut.FourCC);
  ...
  if (m_vppBypass == false) {
    ...
    m_avVppContext = sws_getContext(in.width, in.height, avInFourCC,
                          m_vppFrameInfoOut.Width, m_vppFrameInfoOut.Height, avOutFourCC,
    ...
    if (m_avDecFrameOut) {
      m_avVppFrameIn = m_avDecFrameOut;   // this will go away if we remove Fused ops
    } else {
      ...
      iret = av_image_alloc(m_avVppFrameIn->data, m_avVppFrameIn->linesize, 
                            m_avVppFrameIn->width, m_avVppFrameIn->height,
                            avInFourCC, 16);
    ...
    iret = av_image_alloc(m_avVppFrameOut->data, m_avVppFrameOut->linesize,
                                m_vppFrameInfoOut.Width, m_vppFrameInfoOut.Height, avOutFourCC, 16);
    if(iret < 0)
      return VPL_ERR_INTERNAL_ERROR;
  }
  return VPL_OK;
...
mfxStatus CpuWorkstream::ProcessFrame(vplm_mem *mem)
{
  ...
  vplm_status status = vplm_map_image(mem, VPLM_ACCESS_MODE_READWRITE, &vppCpuImageIn);
  ...
  if (m_vppBypass == true) {
    ...
    sws_scale(m_avVppContext,
              m_avVppFrameIn->data,  m_avVppFrameIn->linesize, 0, m_avVppFrameIn->height,
              m_avVppFrameOut->data, m_avVppFrameOut->linesize);
