...
mfxStatus CpuWorkstream::InitEncode(mfxVideoParam *par)
{
    ...
    if (!m_avEncContext->gop_size)
        m_avEncContext->gop_size = 2 * (int)((float)m_avEncContext->framerate.num / m_avEncContext->framerate.den);
...
mfxStatus CpuWorkstream::EncodeFrame(mfxFrameSurface1 *surface, mfxBitstream *bs)
{
    int err = 0;
