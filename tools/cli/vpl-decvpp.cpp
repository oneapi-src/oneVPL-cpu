...
int main(int argc, char *argv[]) {
    ...
    vpp_data_out = reinterpret_cast<mfxU8 *>(malloc(surface_size * num_surfaces_out));
    vpp_surfaces_out =
        reinterpret_cast<mfxFrameSurface1 **>(malloc(sizeof(mfxFrameSurface1) * num_surfaces_out));

    for (i = 0; i < num_surfaces_out; i++) {
        vpp_surfaces_out[i] =
            reinterpret_cast<mfxFrameSurface1 *>(malloc(sizeof(mfxFrameSurface1)));
