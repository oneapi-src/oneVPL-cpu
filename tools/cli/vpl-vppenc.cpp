...
mfxU16 GetFreeSurfaceIndex(const std::vector<mfxFrameSurface1> &surface_pool);
...
int main(int argc, char *argv[]) {
    ...
    mfxU32 available_surface_index   = 0;
    ...
    while (is_stillgoing == true) {
        ...
        if (is_draining_vpp == false) {
            // Find free frame surface for VPP in
            available_surface_index = GetFreeSurfaceIndex(surfaces_vppin);
            VERIFY(available_surface_index != MFX_ERR_NOT_FOUND, "No available surface for VPP in");

            sts = LoadRawFrame(&surfaces_vppin[available_surface_index], source);
        ...
        if (is_draining_enc == false) {
            ...
            available_surface_index = GetFreeSurfaceIndex(surfaces_vppout_enc);
            VERIFY(available_surface_index != MFX_ERR_NOT_FOUND,
                   "No available surface for VPP out and encode");

            sts = MFXVideoVPP_RunFrameVPPAsync(
                session,
                (is_draining_vpp == true) ? NULL : &surfaces_vppin[available_surface_index],
                &surfaces_vppout_enc[available_surface_index],
                NULL,
                &syncp);
        ...
        if (b_run_encoder == true) {
            sts = MFXVideoENCODE_EncodeFrameAsync(
                session,
                NULL,
                (is_draining_enc == true) ? NULL : &surfaces_vppout_enc[available_surface_index],
                &bitstream,
                &syncp);
...
mfxU16 GetFreeSurfaceIndex(const std::vector<mfxFrameSurface1> &surface_pool) {
    if (surface_pool.size())
        for (mfxU16 i = 0; i < surface_pool.size(); i++)
