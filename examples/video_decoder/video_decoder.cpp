...
mfxStatus VideoDecoder::InitDisplay(void)
{
    ...
    sts = m_session.SetHandle(static_cast < mfxHandleType >(MFX_HANDLE_VA_DISPLAY), m_vaDisp);
...
mfxStatus VideoDecoder::InitSession(mfxU32 bUseExtAlloc)
{
    ...
    printf("MSDK Init - requested API version ... %d.%02d, requested implementation ... 0x%04x\n", m_version.Major, m_version.Minor, m_impl);
    ...
    printf("MSDK Init - actual API version ...... %d.%02d, actual implementation ...... 0x%04x\n", m_version.Major, m_version.Minor, m_impl);
...
mfxStatus VideoDecoder::AllocateSurfaces(void)
{
    ...
    sts = m_mfxVideoDecode->QueryIOSurf(&m_mfxVideoParams, &m_mfxFrameAllocRequest);
    if (sts) {
    ...
    if (m_bUseExtAlloc) {
        ...
        sts = m_mfxFrameAllocator.Alloc(m_mfxFrameAllocator.pthis, &m_mfxFrameAllocRequest, &m_mfxFrameAllocResponse);
        if (sts) {
        ...
        for (surfIdx = 0; surfIdx < m_nSurfaces; surfIdx++) {
            ...
            memcpy(&(m_mfxSurfacePool[surfIdx].Info), &(m_mfxVideoParams.mfx.FrameInfo), sizeof(mfxFrameInfo));
            m_mfxSurfacePool[surfIdx].Data.MemId = &(m_mfxFrameAllocResponse.mids[surfIdx]);
        }
    } else {
        ...
        for (surfIdx = 0; surfIdx < m_nSurfaces; surfIdx++) {
            ...
            m_mfxSurfacePool[surfIdx].Data.Y = m_surfBuf + (surfIdx*surfBytes);
            m_mfxSurfacePool[surfIdx].Data.U = m_mfxSurfacePool[surfIdx].Data.Y + width * height;
            m_mfxSurfacePool[surfIdx].Data.V = m_mfxSurfacePool[surfIdx].Data.U + width * height / 4;
...
{
    if (m_bUseExtAlloc) {
        m_mfxFrameAllocator.Free(m_mfxFrameAllocator.pthis, &m_mfxFrameAllocResponse);
    } else {
        delete [] m_surfBuf;
    }
...
mfxStatus VideoDecoder::DecodeOneFrame(mfxU32 *nBytesBuffered)
{
    ...
    while (nFramesDecoded == 0) {
        ...
        sts = m_mfxVideoDecode->DecodeFrameAsync(m_bDrainingBuffer ? NULL : &m_mfxBitstream, &m_mfxSurfacePool[surfIdx], &m_mfxOutSurface, &syncp);
...
mfxStatus VideoDecoder::WriteFrameToFile(FILE *outfile)
{
    ...
    if (m_bUseExtAlloc) {
        sts = m_mfxFrameAllocator.Lock(m_mfxFrameAllocator.pthis, m_mfxOutSurface->Data.MemId, &(m_mfxOutSurface->Data));
        if (sts) {
    ...
    // write Y plane
    for (y = 0; y < h; y++) {
        offset = pitch * (m_mfxOutSurface->Info.CropY + y) + m_mfxOutSurface->Info.CropX;
    ...
    pitch = m_mfxOutSurface->Data.Pitch;
    for (y = 0; y < h / 2; y++) {
        offset = pitch/2 * (m_mfxOutSurface->Info.CropY + y) + m_mfxOutSurface->Info.CropX;
    ...
    if (m_bUseExtAlloc) {
        sts = m_mfxFrameAllocator.Unlock(m_mfxFrameAllocator.pthis, m_mfxOutSurface->Data.MemId, &(m_mfxOutSurface->Data));
        if (sts) {
            ERR_AND_EXIT("Failed to lock external surface");
        }
    }
