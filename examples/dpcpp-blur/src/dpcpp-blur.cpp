...
mfxU16 ValidateSize(mfxI8 *in, mfxU16 max);
mfxU32 GetSurfaceSize(mfxU32 fourcc, mfxU16 width, mfxU16 height);
mfxU16 GetFreeSurfaceIndex(mfxFrameSurface1 **surface_pool, mfxU16 pool_size);
...
int main(int argc, char *argv[]) {
    ...
    vpp_data_out     = (mfxU8 *)malloc(surface_size * num_surfaces_out);
    ...
    while (is_stillgoing == true) {
        ...
        available_surface_index = GetFreeSurfaceIndex(vpp_surfaces_out, num_surfaces_out);
        VERIFY(available_surface_index != MFX_ERR_NOT_FOUND, "No available surface");
        ...
        switch (sts) {
            case MFX_ERR_NONE:
                ...
                if (syncp) {
                    // VPP output is not available on CPU until
