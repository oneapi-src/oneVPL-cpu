/*############################################################################
  ...
  ############################################################################*/
    ...
    #define TRACE_PLUGIN_ERROR(str, ...) \
        DISPATCHER_LOG_ERROR((("[PLUGIN]: " str), __VA_ARGS__))
    #define TRACE_PLUGIN_INFO(str, ...) \
        DISPATCHER_LOG_INFO((("[PLUGIN]: " str), __VA_ARGS__))

    #define CREATE_PLUGIN_FNC "CreatePlugin"

MFX::PluginModule::PluginModule() : mHmodule(), mCreatePluginPtr(), mPath() {}
...
MFX::PluginModule::PluginModule(const wchar_t *path) : mCreatePluginPtr() {
    ...
    mCreatePluginPtr =
        (CreatePluginPtr_t)mfx_dll_get_addr(mHmodule, CREATE_PLUGIN_FNC);
...
bool MFX::MFXPluginFactory::RunVerification(const mfxPlugin &plg,
    ...
    if (dsc.Default) {
        ...
        if (dsc.PluginVersion > pluginParams.PluginVersion) {
            TRACE_PLUGIN_ERROR(
                "plg->GetPluginParam() returned PluginVersion=%d, but it is smaller than requested : %d\n",
        ...
        if (!dsc.onlyVersionRegistered && pluginParams.CodecId != dsc.CodecId) {
            TRACE_PLUGIN_ERROR(
                "plg->GetPluginParam() returned CodecId=" MFXFOURCCTYPE() ", but registration has CodecId=" MFXFOURCCTYPE() "\n",
                MFXU32TOFOURCC(pluginParams.CodecId),
                MFXU32TOFOURCC(dsc.CodecId));
            return false;
        }

        if (!dsc.onlyVersionRegistered && pluginParams.Type != dsc.Type) {
            TRACE_PLUGIN_ERROR(
                "plg->GetPluginParam() returned Type=%d, but registration has Type=%d\n",
                pluginParams.Type,
                dsc.Type);
            return false;
        }
        ...
        if (pluginParams.PluginUID != dsc.PluginUID) {
            TRACE_PLUGIN_ERROR(
                "plg->GetPluginParam() returned UID=" MFXGUIDTYPE() ", but registration has UID=" MFXGUIDTYPE() "\n",
                MFXGUIDTOHEX(&pluginParams.PluginUID),
                MFXGUIDTOHEX(&dsc.PluginUID));
            return false;
        }

        if (pluginParams.PluginVersion != dsc.PluginVersion) {
            TRACE_PLUGIN_ERROR(
                "plg->GetPluginParam() returned PluginVersion=%d, but registration has PlgVer=%d\n",
                pluginParams.PluginVersion,
                dsc.PluginVersion);
            return false;
        }
        ...
        if (pluginParams.APIVersion.Version != dsc.APIVersion.Version) {
            TRACE_PLUGIN_ERROR(
                "plg->GetPluginParam() returned APIVersion=%d.%d, but registration has APIVer=%d.%d\n",
                pluginParams.APIVersion.Major,
                pluginParams.APIVersion.Minor,
                dsc.APIVersion.Major,
                dsc.APIVersion.Minor);
            return false;
        }
    }
...
bool MFX::MFXPluginFactory::VerifyEncoder(const mfxVideoCodecPlugin &encoder) {
    ...
    return true;
        ...
        #ifndef DISABLE_NON_VPL_DISPATCHER
bool MFX::MFXPluginFactory::VerifyAudioEncoder(
    const mfxAudioCodecPlugin &encoder) {
    if (encoder.EncodeFrameSubmit == 0) {
...
bool MFX::MFXPluginFactory::VerifyDecoder(const mfxVideoCodecPlugin &decoder) {
    ...
    return true;
        ...
        #ifndef DISABLE_NON_VPL_DISPATCHER
bool MFX::MFXPluginFactory::VerifyAudioDecoder(
    const mfxAudioCodecPlugin &decoder) {
    if (decoder.DecodeHeader == 0) {
...
bool MFX::MFXPluginFactory::VerifyCodecCommon(
    const mfxVideoCodecPlugin &videoCodec) {
...
MFX::MFXPluginFactory::MFXPluginFactory(mfxSession session) : mPlugins() {
    mSession = session;
    nPlugins = 0;
}

bool MFX::MFXPluginFactory::Destroy(const mfxPluginUID &uidToDestroy) {
    for (MFXVector<FactoryRecord>::iterator i = mPlugins.begin();
         i != mPlugins.end();
         i++) {
